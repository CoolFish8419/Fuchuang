<template>
	<div>
		...
		>
		<!-- 添加数据对话框表单 -->
		<el-dialog title="新增门店" :visible.sync="dialogVisible" width="40%">
			<el-form
				ref="form"
				:model="form"
				label-width="80px"
			...
			>
				...
				<el-form-item>
					<el-button type="primary" @click="addTable()" :plain="true"
						>提交</el-button
					>
		...
		>
			...
			<div slot="footer" class="dialog-footer">
				<el-button @click="storeInfoDialogVisible = false"
					>取 消</el-button
				>
				<el-button type="primary" @click="submitStoreInfo">
					提 交
				</el-button>
			</div>
		</el-dialog>
...
import { postStore, deleteStore, getStoreByPage, putStore } from "@/apis/store";
export default {
	name: "tableView",
	components: {
		shopButton,
	...
	methods: {
		fetchData(page, pageSize, storeName, address) {
			getStoreByPage({ page, pageSize, storeName, address }).then(
				(res) => {
					//设置表格数据
